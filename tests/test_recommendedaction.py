...
def test_recommended_battle_forecast_text_empty_when_forecast_is_none():
    """``recommended_battle_forecast_text(world, game, player_duchy_id=None)``
    returns ``""`` when
    ``recommended_battle_forecast(world, game, player_duchy_id) is None`` —
    K51.1c.

    Covers no player / unknown id, and non-battle orders
    (``muster`` / ``march`` / ``develop``). Pure and deterministic (no RNG/IO;
    does not mutate ``world`` or ``game``); delegates the forecast decision to
    ``recommended_battle_forecast``.
    """
    recommended_battle_forecast = recommendedaction.recommended_battle_forecast
    ...
    def _assert_empty(world: WorldMap, player_duchy_id: str | None) -> None:
        assert recommended_battle_forecast(world, game, player_duchy_id) is None
        regions_before = world.regions
        parties_before = {r: world.party_at(r) for r in world.regions}
        settlements_before = {
            r: world.settlement_at(r) for r in world.regions
    ...
    # recommended_battle_forecast is None (no player / unknown id)
    none_world = WorldMap(
        [home, enemy_camp],
        [(home, enemy_camp)],
        parties={
    ...
    for player_duchy_id in (None, "missing"):
        assert player_duchy(game, player_duchy_id) is None
        assert recommended_order(none_world, game, player_duchy_id) is None
        _assert_empty(none_world, player_duchy_id)

    # muster → forecast None → ""
    muster_world = WorldMap(
        [keep, enemy_camp],
        [(keep, enemy_camp)],
        parties={
    ...
    assert player_can_muster(muster_world, game, "player") is True
    assert recommended_order(muster_world, game, "player") == ("muster", None)
    _assert_empty(muster_world, "player")
    ...
    assert target is not None
    assert recommended_order(march_world, game, "player") == ("march", target)
    _assert_empty(march_world, "player")

    # develop → forecast None → ""
    develop_world = WorldMap(
        [home],
        [],
        parties={home: Party(hero=Unit(), units=(), owner_id="player")},
