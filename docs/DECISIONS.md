...
| K39.1a | Prymityw alertu zagrożeń (pusty korzeń) | `tbbui.threatalert.render_threat_alert(world, game, player_duchy_id=None)` → `<div data-threat-alert="">`; gdy `player_duchy(...) is None` → sam pusty korzeń (bez `data-threats`, bez tekstu, bez dzieci). |
