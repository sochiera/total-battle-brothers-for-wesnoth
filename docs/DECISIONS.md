...
|----|-------|-----------------|
| K16.1c | Prymityw AI szturmu na wskazaną osadę zwraca bitwę | `ai.assault_duchy_party_to_recorded(world, duchy, target, rng, morale_by_owner=None) -> (WorldMap, HexBattle \| None)` reużywa `resolve_settlement_battle_recorded` (mapa = `assault_duchy_party_to`); no-op → `(world, None)` bez RNG. |
