...
| `game/scenes/main.tscn` | Scena startowa: korzeń `Main` (`Control`) ze skryptem `res://scripts/main.gd` oraz pustymi kontrolkami `DateLabel` (`Label`), `RegionList` (`ItemList`), `ResultLabel` (`Label`), `PlayerDuchyStatusLabel` (`Label`) i `NextTurnButton` (`Button`) — w tej kolejności. `PlayerDuchyStatusLabel` rezerwuje nazwane miejsce na status księstwa gracza. |
...
(`ItemList`), `ResultLabel` (`Label`), `PlayerDuchyStatusLabel` (`Label`) i
`NextTurnButton` (`Button`) pozostają w tej kolejności. `PlayerDuchyStatusLabel`
