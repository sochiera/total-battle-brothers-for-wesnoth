...
(ściśle większe; bilans równy nie liczy się; inne księstwa ignorowane). Gdy
`player_duchy_id` jest `None` albo spoza `game.duchies` — sam pusty korzeń
(bez `data-starving-settlements`). Czyste, deterministyczne, bez mutacji
`game`; rdzeń bez zmian. Reużywa `tbbui.gamelookup.player_duchy`.
...
│       ├── economyalert.py # HTML alert gospodarczy: głodujące osady (K60.1a)
...
│   ├── test_economyalert.py # HTML alert gospodarczy (tbbui, K60.1a)
