...
| K28.1b | Skutek recruit/muster/develop | `_apply_player_order(transition, label)` ustawia `last_notice` na `{label}: wykonano` gdy `world` się zmienił, inaczej `{label}: brak zmian` (w tym guardy); etykiety: Rekrutacja / Zebranie oddziału / Rozbudowa. |
