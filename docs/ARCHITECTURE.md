...
| `game/scripts/order_result.gd` | Czysta projekcja wyniku rozkazu: `OrderResult.from_response(response: Dictionary) -> Variant` zwraca dokładnie `{"order": String, "changed": bool}` tylko dla kompletnego `ok:true` i `result.kind == "order"`; brak lub niewłaściwy typ wymaganego pola zwraca `null`. |
