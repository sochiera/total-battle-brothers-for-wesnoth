...
| `game/scripts/bridge_config.gd` | Czysta walidacja konfiguracji startowej: `BridgeConfig.from_values(command, state_path, seed_text)` obcina białe znaki brzegowe z komendy i ścieżki stanu, po czym zwraca wyłącznie słownik `{command, state_path, seed}`. Puste wartości oraz seed, który nie jest poprawną liczbą całkowitą (ujemną wolno), dają atomowo `null`. |
