...
| `game/scripts/order_result.gd` | Czysta projekcja wyniku rozkazu: `OrderResult.from_response(response: Dictionary) -> Variant` zwraca dokładnie `{"order": String, "changed": bool}` tylko dla kompletnego `ok:true` i `result.kind == "order"`; dla kompletnego `result.kind == "battle"` zwraca projekcję z `kind`, `order`, `outcome` oraz stratami obu stron. Brak lub niewłaściwy typ wymaganego pola zwraca `null`. `status_text()` formatuje zwykłe rozkazy, w tym szturm, albo projekcję bitwy szturmu jako wynik, straty własne i wroga; nierozpoznany kształt daje pusty tekst. `failure_status_text()` zwraca ogólny polski komunikat nieudanego rozkazu. |
