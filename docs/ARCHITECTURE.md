...
**Alert gospodarczy HTML (K60.1a / K60.1b):**
...
(ściśle większe; bilans równy nie liczy się; inne księstwa ignorowane), oraz
widoczny tekst `Osady na deficycie pszenicy: N` spójny z atrybutem. Gdy
`player_duchy_id` jest `None` albo spoza `game.duchies` — sam pusty korzeń
(bez `data-starving-settlements` i bez tekstu). Czyste, deterministyczne, bez
mutacji `game`; rdzeń bez zmian. Reużywa `tbbui.gamelookup.player_duchy`.
...
│       ├── economyalert.py # HTML alert gospodarczy: głodujące osady (K60.1a–b)
...
│   ├── test_economyalert.py # HTML alert gospodarczy (tbbui, K60.1a–b)
