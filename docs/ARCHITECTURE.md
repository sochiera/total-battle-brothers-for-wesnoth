...
| `game/scripts/bridge_batch_probe.gd` | Samoweryfikująca sonda czystej warstwy wsadowej JSON Lines: sprawdza odwracalność `request_lines`, pusty wsad oraz kompletność, kolejność i zatrzymanie `all_responses`, po czym wypisuje `BRIDGE_BATCH <json>`. Opcja `--corrupt` celowo psuje wynik kontroli i kończy proces kodem `1`. |
| `game/scripts/bridge_call_probe.gd` | Sonda wywołania mostu: przyjmuje komendę mostu i opcjonalną ścieżkę żądania, wywołuje `BridgeClient.send({"type": "snapshot"})` i wypisuje `BRIDGE_CALL <json>` z `is_null`, posortowanymi `keys` oraz `ok`; kończy kodem `0`. Bez komendy raportuje stderr i kończy kodem `2`. |
| `game/scripts/bridge_many_probe.gd` | Sonda integracyjna wsadu: uruchamia `send_many` dla dwóch `next_turn`, bezwarunkowo odrzuca niekompletną odpowiedź i wypisuje `BRIDGE_MANY <json>` wyłącznie dla kompletnego wsadu. Opcje `--single` i `--empty` porównują pojedynczy wsad z `send` oraz sprawdzają wczesny zwrot pustego wsadu; `--corrupt` celowo usuwa odpowiedź i kończy się kodem `1`, a `--allow-empty` jest przeznaczone tylko do obserwacji oczekiwanych awarii klienta. |
