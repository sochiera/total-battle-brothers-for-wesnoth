...
## One-shot review capture for G100.1c (task-568). Run with a real display
...
const EMPTY_PATH := "res://screenshots/task-568-selected-region-empty-1152x648.png"
const SELECTED_PATH := "res://screenshots/task-568-selected-region-selected-1152x648.png"
...
func _run() -> void:
	...
	if scene == null:
		printerr("capture_selected_region_review: cannot load main scene")
		quit(2)
		return

	var scene_root: Control = scene.instantiate() as Control
	if scene_root == null:
		printerr("capture_selected_region_review: cannot instantiate main scene")
		quit(2)
		return
	...
	if not scene_root.has_method("apply_model"):
		printerr("capture_selected_region_review: Main.apply_model missing")
		quit(3)
		return
	...
	# Empty selection: compact „Nie wybrano regionu” hierarchy.
	if not _save_viewport(EMPTY_PATH):
		quit(4)
		return
	print("CAPTURED ", EMPTY_PATH)

	# Selected player region: four distinct rows + map target frame.
...
func _save_viewport(res_path: String) -> bool:
	...
	if tex == null:
		printerr("capture_selected_region_review: viewport texture null (use non-headless)")
		return false
	var img: Image = tex.get_image()
	if img == null:
		printerr("capture_selected_region_review: get_image null")
		return false
	if img.get_width() != int(VIEWPORT_W) or img.get_height() != int(VIEWPORT_H):
		printerr(
			"capture_selected_region_review: unexpected size %sx%s"
	...
	if err != OK:
		printerr("capture_selected_region_review: save_png failed ", err, " ", abs_path)
		return false
	return true
...
func _click_region(map_view: Node, region_name: String) -> void:
	...
	if tile == null:
		printerr("capture_selected_region_review: tile missing for ", region_name)
		return
