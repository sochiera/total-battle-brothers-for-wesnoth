...
def test_render_situation_report_threatened_count_and_label():
    """Known player: root has ``data-threatened-count="N"`` and visible text
    ``Sytuacja: zagrożone pozycje N``. ``N`` counts own positions (player
    settlement and/or party, separately per region and kind) that have ≥1
    neighbor with ``party_at(neighbor).owner_id`` set and ``!= player_duchy_id``
    — same rule as ``render_threat_alert``. Pure (no world/game mutation).
    """
    home = Region("Home")
    keep = Region("Keep")
    fort = Region("Fort")
    ...
    assert root.text == f"Sytuacja: zagrożone pozycje {expected_n}"
    assert list(root) == []
...
def test_render_situation_report_count_from_threatened_position_count():
    ...
    Known player with zero threats still yields ``data-threatened-count="0"``
    and matching label; unknown player: helper returns 0 while report stays empty.
    ...
    assert root.text == f"Sytuacja: zagrożone pozycje {n}"
    ...
    assert zero.text == "Sytuacja: zagrożone pozycje 0"
