...
| `game/scenes/main.tscn` | Scena startowa: korzeń `Main` (`Control`) ze skryptem `res://scripts/main.gd` oraz pustymi kontrolkami `DateLabel` (`Label`), `RegionList` (`ItemList`), `ResultLabel` (`Label`), `PlayerDuchyStatusLabel` (`Label`), `NextTurnButton` (`Button`) i `DevelopButton` (`Button`) — w tej kolejności. `PlayerDuchyStatusLabel` rezerwuje nazwane miejsce na status księstwa gracza, a `DevelopButton` (`Rozwiń osadę`) jest na razie niepodłączoną kontrolką pierwszego rozkazu. |
| `game/scripts/main.gd` | Skrypt głównej sceny (`extends Control`); w `_ready()` odczytuje `BridgeConfig.from_environment()` i tylko dla konfiguracji różnej od `null` wywołuje `start_session(config)`. Publiczne wejście startowe stanowią `TBB_BRIDGE_COMMAND`, `TBB_STATE_PATH` i `TBB_SEED`; bez kompletnej poprawnej konfiguracji wejście sceny do drzewa jest no-opem. `start_session(config) -> bool` deleguje walidację gotowego słownika `{command, state_path, seed}` do `BridgeConfig.is_valid_session_config`, następnie tworzy trwałego klienta przez `BridgeClient.create_persistent`, wiąże go i renderuje świeży snapshot bez przesuwania tury. Nieprawidłowa konfiguracja zwraca `false` bez modyfikacji sceny. Wejście prezentacji `apply_model(model: SnapshotModel)` wiąże datę, nazwy regionów, wynik i status księstwa gracza idempotentnie: status ma format `Morale: <morale>, osady: <settlements>, oddziały: <parties>`, a brak statusu ustawia pusty stan zamiast zachowywać poprzedni tekst; przed wypełnieniem czyści `RegionList`. `refresh_from_bridge(client) -> bool` pobiera od wstrzykniętego klienta model i wiąże go tylko przy sukcesie; `advance_turn_from_bridge(client) -> bool` jednokrotnie wykonuje `client.advance_turn()` i wiąże zwrócony model tylko przy sukcesie. Obie metody zwracają `false` i zostawiają kontrolki nietknięte przy `null`. `bind_client(client)` zapamiętuje klienta i utrzymuje dokładnie jedno połączenie `NextTurnButton.pressed`; handler przesuwa turę na ostatnio związanym kliencie. |
...
`NextTurnButton` (`Button`) i `DevelopButton` (`Button`) pozostają w tej kolejności.
`PlayerDuchyStatusLabel`
wiąże morale, osady i oddziały księstwa wskazanego przez model. **Poza zakresem
bootstrapu:**
