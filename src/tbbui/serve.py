...
def recommended_order_path(action: str) -> str:
    ...
    their names; ``defend`` and ``march`` reuse ``/order/march`` (defend =
    move to a threatened position; march = approach a distant enemy
    settlement). Unknown actions raise ``KeyError``.
    """
...
class GameApp:
    ...
    def _apply_player_assault_order(self, transition, label: str) -> None:
        ...
        when a battle was recorded (K46.1b / K46.2b / K47.1b), else
        ...
        if battle is not None:
            ...
            self.last_notice = (
                ...
                f"(straty: {attacker_losses(battle)}, wróg: {defender_losses(battle)})"
            )
        else:
            self.last_notice = f"{label}: brak zmian"
        self.game = self.game.sync_from_world(self.world)
    ...
    def _recommended_order_form(self) -> str:
        ...
        reason_html = _escaped_paragraph("recommended-order-reason", reason)
