...
| K51.1e | Prognoza siły przy przycisku rady w jeden klik | `GameApp._recommended_order_form()` przy emitowanym formularzu i niepustej prognozie dokłada po `data-recommended-order-reason` jedno `<p data-recommended-order-forecast="{text}">{text}</p>` (`html.escape(quote=True)`); pusta prognoza / brak formularza → brak elementu. |
